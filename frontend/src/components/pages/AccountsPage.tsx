...
const AccountsPage = () => {
  ...
  return (
    <div className="p-4 flex flex-col gap-6">
      <div>
        <h2 className="text-4xl font-medium mb-2">Accounts</h2>
        <p className="text-gray-500">
          Manage and view all your connected financial accounts
        </p>
      </div>
                ...
                <div key={type}>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-xl font-semibold capitalize">{type} Accounts</h3>
                    ...
                    {accountList.map((acc) => {
                      ...
                      return (
                        <div
                          ...
                          className={`border rounded-lg p-4 ${getAccountColor(acc.type)} transition-all hover:shadow-lg cursor-pointer`}
                        >
                          <div className="flex justify-between items-start">
                            <div className="flex gap-3 items-start flex-1">
                              <div className="mt-1">
                                <AccountIcon className="text-gray-600" fontSize="medium" />
                              </div>
                                ...
                                <div className="flex gap-4 text-xs text-gray-500">
                                  {acc.balances.available !== null && acc.balances.available !== undefined && (
                                    <div>
                                      ...
                                      {acc.balances.available.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </div>
                                  )}
                                  {acc.balances.limit && (
                                    <div>
                                      <span className="font-medium">Limit:</span> $
                                      {acc.balances.limit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </div>
                                  )}
                                  {acc.holder_category && (
                                    <div>
                                      <span className="font-medium">Type:</span>{" "}
                                      {acc.holder_category.charAt(0).toUpperCase() + acc.holder_category.slice(1)}
                                    </div>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="text-right">
                              <p className={`text-2xl font-bold ${isNegative ? "text-red-600" : "text-gray-900"}`}>
                                {isNegative ? "-" : ""}$
                                {Math.abs(acc.balances.current || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </p>
      ...
      {selectedAccount && (
        ...
        >
          ...
          >
            {/* Modal Header */}
            <div className={`p-6 border-b ${getAccountColor(selectedAccount.type)}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {selectedAccount.official_name || "No official name"} • ••••{selectedAccount.mask}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="grid grid-cols-3 gap-4 mt-6">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    ${(selectedAccount.balances.current || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </p>
                </div>
                {selectedAccount.balances.available !== null && selectedAccount.balances.available !== undefined && (
                  <div>
                    <p className="text-sm text-gray-600">Available Balance</p>
                    <p className="text-xl font-semibold text-gray-700">
                      ${selectedAccount.balances.available.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  </div>
                )}
                {selectedAccount.balances.limit && (
                  <div>
                    ...
                    <p className="text-xl font-semibold text-gray-700">
                      ${selectedAccount.balances.limit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-6 mt-4 text-sm">
                <div>
                  ...
                  <span className="font-medium">{getAccountTypeLabel(selectedAccount.type, selectedAccount.subtype)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Currency:</span>{" "}
                  <span className="font-medium">{selectedAccount.balances.iso_currency_code || "USD"}</span>
                </div>
                {selectedAccount.holder_category && (
                  <div>
                    <span className="text-gray-600">Category:</span>{" "}
                    <span className="font-medium capitalize">{selectedAccount.holder_category}</span>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {accountTransactions.slice(0, 20).map((txn) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className={`font-semibold ${txn.amount > 0 ? "text-red-600" : "text-green-600"}`}>
                          {txn.amount > 0 ? "-" : "+"}$
